...
/* Wave 56, W56-H. Both transcribed from their PROMOTED definitions
 * (src/decomp/c_0801F48C.c, src/decomp/c_08024378.c) rather than from
 * sub_08027FF4's call sites; neither had a declaration. */
...
/* Signature copied from the PROMOTED definition in src/decomp/c_080136DC.c, not
 * derived from a call site: `pal` is SIGNED 8-bit and its sign bit is a flag
 * (the body shares one `lsls #0x18` between the `lsrs #0x18` value store and an
 * `asrs #0x1f` brightness bias). Declared here in wave 56 (W56-P) because
 * sub_0806B3E8 is the first caller outside its own translation unit. */
 ...
 *
 * WAVE 38 (W38-J): the object IS modelled -- it is `struct Unit`, and
...
/* Wave 54, W54-G. PROMOTED-but-undeclared: the body has been in
 * src/decomp/c_0802BAFC.c since it was matched, and the definition is copied
 * here verbatim rather than re-derived from the call site -- the definition is
 * the stronger witness. Its caller sub_0802B4D4 corroborates it: both halfword
 * arguments are narrowed `lsls #0x10; lsrs #0x10` immediately before the `bl`,
 * which is what a prototyped `u16` parameter costs a caller, and the third
 * arrives as a bare `subs r2, r2, r1` with no re-narrowing at all. */
void sub_0802BAFC(u16, u16, int);
/* Wave 54, W54-G. MATCHED, and the signature is settled by the match rather
 * than guessed: all three parameters carry the prologue `lsls #16; lsrs #16`
 * that a declared narrow parameter costs, and each is then sign-extended again
 * at its uses (`lsls #16; asrs #16`) -- the PROMOTE_MODE-plus-extend-at-use
 * shape of an `s16`, not of a `u16`. a1 is an x offset, a2 a y offset and a3
 * the row index into gUnknown_0849A2A6. Void: sub_0802AA78 discards nothing and
 * the epilogue is `pop {r0}`. */
void sub_0802B4D4(s16, s16, s16);
/* Wave 56, W56-O. Declared from the CALL SITE, not matched -- sub_0802B3AC
 * (296 B, status `asm`) is sub_0802AA78's immediate successor and its only
 * caller is sub_0802AA78, at 0x0802B0AC, where it is handed the SAME three
 * registers sub_0802B4D4 above is handed two instructions earlier:
 *   asrs r4,r0,#0x10 / asrs r5,r1,#0x10 / asrs r6,r2,#0x10
 *   adds r0,r4,#0; adds r1,r5,#0; adds r2,r6,#0; bl sub_0802B4D4
 *   adds r0,r4,#0; adds r1,r5,#0; adds r2,r6,#0; bl sub_0802B3AC
 * so the arity is 3 and the twin's already-matched `s16, s16, s16` is the
 * strongest witness available for the widths.  (`s16` and `int` are
 * byte-neutral AT THIS CALL SITE: `(s32)x >> 16` is provably in s16 range, so
 * agbcc emits no narrowing for either spelling -- agreeing with the twin is
 * the evidence, not the instruction stream.)  VOID: the caller's very next
 * instruction is `ldr r2,=gDispIo; ldrb r0,[r2,#1]`, i.e. r0 is overwritten
 * without being read, so the return value is discarded. */
void sub_0802B3AC(s16, s16, s16);
/* Wave 56, W56-Q. PROMOTED-but-undeclared, all six: every signature below is
 * copied VERBATIM from the byte-exact definition named after it, not derived
 * from sub_0802AA78's call sites -- the definition is the stronger witness
 * (wave 30). sub_0802AA78 is the first caller of any of them to need a
 * prototype, so adding these regresses nothing: src/, include/ and work/ were
 * all swept and no other file declares or calls them. */
...
/* Wave 54, W54-G. The 0x08055/0x08057 cutscene-player helpers sub_080553C8
 * drives, none of which had a declaration even though two of them have bodies.
 * sub_080555F0 is copied verbatim from its PROMOTED definition in
 * src/decomp/c_080555F0.c; sub_08055654, sub_08055940 and sub_08057164 are
 * copied from the unpromoted drafts in work/, which are the shared model for
 * them. sub_080553C8 is the only caller of any of the five in the whole tree
 * (src/, include/ and work/ all swept), so adding these regresses nothing.
 *   sub_08055D4C has neither a body nor a draft: its signature is read off
 * sub_080553C8's call site, which builds the identical two-`ldrh` argument list
 * from gUnknown_02029BE8[0] and [1] that the sub_08055654 and sub_08055940
 * calls on the two lines above it build, and discards the result. Weakest type
 * that fits the pair; the return being void is from the discard alone. */
/* Wave 54, W54-G. Three more PROMOTED-but-undeclared callees, all reached from
 * sub_0801E508 and copied verbatim from their definitions -- sub_0801E3B4 from
 * src/decomp/c_0801E3B4.c and sub_0808B710 (the ROM's sinf) from
 * src/decomp/c_0808B710.c. sub_0808B91C has no promoted body; its `float
 * sub_0808B91C(float)` comes from the draft in work/sub_0808B91C, which the
 * call site corroborates exactly -- sub_0801E508 hands it a `__floatsisf`
 * result and feeds the return straight into `__mulsf3`, and it is the cosf to
 * sub_0808B710's sinf (the two are called in matched pairs on the same angle).
 * sub_0801E508 is the only caller of any of the three outside their own files,
 * so declaring them regresses nothing. */
int sub_0801E3B4(int);
float sub_0808B710(float);
float sub_0808B91C(float);
/* Wave 54, W54-G. PROMOTED-but-undeclared, copied verbatim from the definition
 * in src/decomp/c_08016944.c. sub_0801E9B0 is its only caller anywhere in the
 * tree (src/, include/ and work/ swept), so declaring it regresses nothing. */
...
/* Two more of the 0x08079xxx proc helpers, both void, both taking TWO
 * arguments and both taking the scroll offset second as `u32` (the guards are
 * `cmp rN, #imm; bhi`, an unsigned ordering an `int` cannot produce).
 * sub_080795A8 opens `adds r4, r1, #0` and then writes r0 before reading it,
 * so its proc pointer is genuinely unused and the `movs r1, #0` at the call is
 * the arity evidence, not the pointer. sub_08079EA4 is a caller of both.
 *
 * sub_08079B04 WAS `sub_08079B38` UNTIL WAVE 54 (W54-E), AND THE RENAME IS THE
 * FIX, not a workaround. 0x08079B38 is the entry; the 52 bytes in front of it
 * are sub_0807974C's literal pool, which agbcc emits past that function's
 * `bx r0` behind a `b` -- so the pool is stored inside the address range the
 * splitter assigned to the NEXT function and tools/split_asm.py read the `b`
 * as a function start. Compiled as ONE translation unit with sub_0807974C in
 * address order (which is how it must be promoted anyway), agbcc puts the pool
 * exactly there and the label `sub_08079B04` lands on 0x08079B38 by itself. No
 * splitter change and no lds symbol are needed. The three callers'
 * `bl _08079B38` then assembles as a local, unrelocated branch, which was the
 * entire 2-byte residual of sub_08079EA4, sub_08079FAC and sub_0807A0C4.
 * `trymatch --unit sub_08079B04` is the oracle; the per-function oracle cannot
 * pass either half and its percentage is not evidence about the source. First
 * argument IS used here, unlike sub_080795A8's. */
 ...
 * which is 6 bytes wrong. Its three callers carry their own `static` prototype
 * and are all inside the same unit. */
...
/* Wave 56 (W56-F): the one-argument member of the same gate family, needed by
 * sub_0801ADC8. Transcribed from the promoted definition in
 * src/decomp/c_0801B598.c, not inferred -- `bl _call_via_r1` there fixes the
 * arity at one and the `lsls #0x10; lsrs #0x10` pair the parameter at u16. */
...
/* Wave 54 (W54-B). One pointer argument and nothing else: sub_08089A04's only
 * call site is `adds r0, r6, #0; bl sub_08088ECC` passing the same proc it was
 * itself called with, and sub_08088ECC's own prologue reads r0 before writing
 * it (`adds r7, r0, #0`) and no other argument register at all. `void *` for
 * the same reason as sub_08089464 above -- the proc struct is described only
 * locally in the files that use it. Nothing consumes a return value at either
 * call site. */
...
/* Wave 54 (W54-B). The rest of sub_0808844C's dispatch table, declared the same
 * way as sub_080895E4 above: each already has a promoted definition whose proc
 * tag is TU-local, so the tag is forward-declared here and sub_0808844C casts
 * to the incomplete type instead of restating a layout it does not own. The
 * definitions are the witnesses -- src/decomp/c_08088CDC.c for the first two,
 * c_08089C14.c for the next two, c_0808A2F4.c for the last -- and every one of
 * them takes exactly one proc pointer and returns void, which is also what
 * sub_0808844C's call sites show (`adds r0, r7, #0; bl ...`, nothing consumed
 * afterwards). sub_080897C8 and sub_08089A04 are wave-54 drafts in
 * work/, not yet promoted; their tags follow the same convention. */
struct Unk08088CDC;
...
/* Wave 56 (W56-B). The record-screen redraw, declared the same way as
 * sub_080895E4 above: src/decomp/c_0807DA98.c DEFINES it and owns the tag body,
 * so only the tag is forward-declared here and its sole caller sub_0807CE5C
 * casts to the incomplete type.
 *
 * `void` is settled from the call site, not from the body: sub_0807CE5C does
 * `adds r0, r7, #0; bl sub_0807DA98` and its very next instruction is
 * `ldr r0, [r7, #0x3c]`, so r0 is overwritten without being read.
 *
 * The object is the SAME one sub_0807D800, sub_0807D860 and sub_0807D918 take
 * -- all three promoted, all three modelling it TU-locally under their own tags
 * (Unk807D800 / Unk807D860 / Unk7D918) with mutually consistent members. This
 * function sees the widest window of the four and its tag agrees with all three
 * where they overlap (s16 +0x4c, u16 +0x52, int +0x58/+0x5c/+0x60, s16 +0x64),
 * adding int +0x2c..+0x40, u16 +0x48, s16 +0x4e, s16 +0x66 and s16 +0x6a. Those
 * four tags are one struct seen four ways and are worth unifying, but doing it
 * would retype three already-matched functions, which the shared-member rule
 * forbids doing unilaterally -- recorded here rather than done.
 *
 * The +0x60 member is `volatile int` in c_0807DA98.c and that is load-bearing:
 * the ROM contains a dead `ldr r0,[rN,#0x60]` that flow deletes without it.
 * See docs/agbcc-codegen.md, "A dead load that survives -O2". */
struct Unk7DA98;
...
/* Wave 56 (W56-N), the nine handlers sub_080706B0 (MPlayExtender) patches into
 * the m4a jump table at &gUnknown_03005740 -- indices 8, 17, 19 and 28..33, in
 * that order, which is exactly MP2K's ply_memacc / ply_lfos / ply_mod /
 * ply_xcmd / ply_endtie / SampleFreqSet / TrackStop / FadeOutBody /
 * TrkVolPitSet. Four of the nine are already declared above or promoted
 * (sub_080702C0, sub_080708EC, sub_0807004C, sub_080717C4); the six below are
 * the remainder.
 *
 * ONLY THEIR ADDRESSES ARE TAKEN, so ARITY IS NOT PROVED for any of them --
 * nothing in C calls them yet and the table slot is written through a cast,
 * which is byte-neutral whatever the shape. `void (void)` is the weakest shape
 * that fits, the same choice sub_0806F7C8 above makes for the same reason.
 * Whoever matches one of these bodies should retype it from the body and fix
 * the cast in src/decomp/c_080706B0.c; proto_check will not catch it, because a
 ...
 * rather than lifted from the call site -- src/decomp/c_08070EF4.c,
 * c_08070E4C.c and c_080717C4.c. They were not declared anywhere before, which
 * is why nothing outside their own units could take their addresses.
 * sub_080717C4's second parameter type is file-local to c_080717C4.c, so the
 * tag is forward-declared here and completed there. */
void sub_08070EF4(u8);
/* Wave 56 (W56-N): CgbModVol, MP2K's per-channel volume/panning recompute.
 * PARKED, not matched, so this is read off the CALL SITES rather than a
 * promoted body -- sub_08070FAC calls it twice, both times `adds r0,r4,#0` with
 * r4 the CgbChannel cursor and r0 dead afterwards, so one pointer argument and
 * void. It agrees with the parked draft in work/sub_08070F44/. */
...
/* Matched and promoted in src/decomp/c_080039E4.c with this exact signature;
 ...
 * The three sub_080433xx accessors below took a file-local `struct Unk43304`
 * in src/unit.c until they were RETYPED to the shared struct BattleUnit, on
 * the same grounds as sub_08024ED8 below: every call site is in src/battle.c
 * and passes gBattleAttacker or gBattleDefender, and the two models agree
 * where they overlap -- Unk43304's `unk00` is BattleUnit's `unit` and its
 * `unk06` is `terrainDefense`, the field sub_08024C58 assigns the result of
 * sub_08043304 back into. Byte-verified against the canonical ROM. */
struct BattleUnit;
...
/* Wave 56, W56-E. All three were undeclared. The first two are COPIED FROM
 * THEIR PROMOTED BODIES (src/decomp/c_08029D1C.c, c_08029DBC.c), which are the
 * stronger witness; neither is derived from a call site.
 *   sub_08029DF8 is `void`: it ends `pop {r0}; bx r0`, the void epilogue that
 * c_08029D1C.c's own comment contrasts with its `pop {r1}; bx r1`. Its one
 * parameter is the proc pointer -- +0x1e and +0x20 are read with the
 * `movs rI,#0x1e; ldrsh` idiom struct Unk03001470 records for both. */
int sub_08029D1C(void *);
int sub_08029DBC(int, int);
void sub_08029DF8(struct Unk03001470 *);
/* Wave 56, W56-E. sub_08029FC4 was undeclared despite src/decomp/c_08029FC4.c
 * defining it; copied from there. sub_08029FE4 takes nothing (no argument
 * register is read before being written) and ends `pop {r0}; bx r0`, the void
 * epilogue. */
 ...
 *   void sub_080201E0(s16, s16, struct Unit *);  c_080201E0.c
 *   int  sub_0803E9F8(struct Unk0803E9F8 *, u8 *, u8, u8);  c_0803E9F8.c
 *   int  sub_08041FE0(struct Unit *);        c_08041F38.c
 *   int  sub_0804203C(struct Unit *);        c_0804203C.c
 *
 ...
 *   The two 0x08042xxx functions take `struct Unit *`, not
...
/* Wave 56, W56-I -- five more entry points sub_0802FACC reaches.
 *
 * sub_0802FA9C and sub_08030038 are copied VERBATIM from their promoted
 * definitions (src/decomp/c_0802FA9C.c, src/decomp/c_08030038.c), which are the
 * stronger witness; nothing here is derived from the call site.
 *
 * sub_0802EA24: nullary void.  sub_0802FACC calls it three times with no
 * argument setup at all and the result is never read -- the very next
 * instruction on every path reloads gUnknown_0849B018 into r0.
 *
 * sub_0802F6A0: the receive-queue pop.  `s16` return, from the caller's
 * `lsls #0x10; asrs #0x10` re-narrowing before the `> 0` test and the switch on
 * 4 / 0x1a / 0x84 / 0x86 -- agbcc re-narrows a narrow-returning callee at every
 * call site, and a plain `int` would emit neither shift.  First parameter s8
 * (`lsls #0x18; asrs #0x18` on the loop counter at the call), second is the
 * packet buffer at gUnknown_0849B018 + 0x2c, spelled `void *` here because
 * sub_0802FACC reads that buffer through four different packet layouts and no
 * one of them is the callee's view.  A pointer parameter is byte-neutral in the
 * caller whatever it points at.
 *
 * sub_080307E0: takes the address of a caller local; the out parameter is a
 * word (`ldr r1, [sp]`) and is a LENGTH -- sub_0802FACC passes `(u16)(len + 6)`
 * as sub_0802F588's byte count.
 *   The RETURN TYPE IS FROM THE PROMOTED BODY (src/decomp/c_080307E0.c), not
 * from the call site.  Call-site evidence alone says struct Unk0202575C *,
 * because sub_0802FACC hands the return straight to sub_0802F588 with no
 * instruction in between and that is sub_0802F588's declared first parameter --
 * and that reading is WRONG.  The body returns
 * `&gUnknown_0849B018->unk12c[unk1aac]`, a struct Unk08090CD8Entry *, so the
 * caller has to cast.  Caught by proto_check --all; recorded because the wrong
 * inference is the natural one to make here twice over: the 0x2c packet buffer
 * sub_0802FACC reads in the same function IS an Unk08090CD8Entry, so the two
 * types genuinely both appear. */
 ...
 * `u8 (struct Unit *, s16, s16)`, not `(int, int, int)`.
...
/* Wave 56 (W56-N): the three neighbours sub_0806DFF0 calls. All three already
 * have PROMOTED bodies -- src/decomp/c_0806DE38.c, c_0806DF20.c and
 * c_0806DF58.c -- and these are transcribed from those definitions, which is
 * the stronger witness than the call site. None was declared anywhere before. */
...
/* Wave 54 (W54-D). sub_08086688 is the first C caller any of these six ever
 * had, and -Werror makes an implicit declaration fatal, so they are published
 * here rather than left implicit. Four of them already have a body and the
 * DEFINITION is what is copied, not a caller-side guess:
 *   sub_080870B8  src/decomp/c_080870B8.c   void(int,int,int,int)
 *   sub_08087168  src/decomp/c_08087104.c   void(int)
 *   sub_08087220  src/decomp/c_08087220.c   void(int,int)
 *   sub_080867BC  src/decomp/c_080867BC.c   void(struct Unk080867BCProc *)
 * sub_080867BC's parameter struct is file-local to c_080867BC.c by the
 * parameter-struct rule, so only a forward declaration of the tag lives here --
 * the same shape as sub_0808606C above. ProcPtr is `void *`, so sub_08086688
 * passes its own parameter straight through with no cast and no instruction.
 * sub_08087040 has no promoted body yet; the ROM sets up no argument register
 * before the `bl` and it `pop {r0}`s, so `void(void)`, which is also what
 * work/sub_08087040 drafts. sub_080872D0 is caller-side only: one whole-word
 * `ldr` of the proc's +0x5c and nothing after the `bl`, i.e. void(int) -- the
 * same argument sub_08087168 takes at the immediately preceding call. */
...
/* Wave 54 (W54-D) RETYPED argument 2 from `u32` to `int`, from BODY evidence
 * the caller side could not have: 0x08086A58 tests it three times and every
 * one of the three is a SIGNED branch -- `cmp r7,#0; ble` on each loop guard
 * and `cmp r1,r7; bge` against gUnknown_02027F74.unk37. A `u32` parameter turns
 * all three into `bcs`/`bcc` and also blocks check_dbra_loop, so both counted
 * loops keep an ascending index and an extra live register instead of the ROM's
 * down-counter. W44-C's "gUnknown_03005928 is declared u32" was the only
 * evidence for the old type and it is caller-side and byte-neutral: u32 -> int
 * converts with no instruction, so nothing at a call site changes.
 * Callers chased and re-verified by exit code: src/decomp/c_0808606C.c
 ...
 * unchanged (`mov r0,sl; adds r1,r7,#0; mov r2,sb`). */
void sub_08087548(int, int, int);
/* Wave 54 (W54-D). Both DEFINED in src/decomp (c_08085950.c, c_080858C0.c) and
 * never declared until sub_080849C8 and sub_08084C14 became their first C
 * callers; the DEFINITIONS are what is published. */
void sub_08085950(int, int);
void sub_080858C0(void);
/* Wave 54 (W54-D). The 0x08085 tree's per-mode redraw quartet plus two more,
 * all first called from C by sub_08084C14 this wave. The four `s16 *` are the
 * DEFINITIONS in src/decomp/c_08085168.c, c_080851CC.c, c_08085208.c and
 * c_08085244.c -- that family reaches the proc as a halfword array (`p[0x33]`
 * is +0x66), and sub_08084C14 does the same, so no cast is needed at any of
 * the four call sites. sub_08085908 is `void(void)` from c_08085908.c.
 * sub_08084F44 and sub_08085044 keep their file-local parameter structs per
 * the parameter-struct rule, so only the tags are forward-declared here;
 * sub_08085044's body is work/sub_08085044 (matched this wave, not yet
 * promoted) and its tag is `struct Unk8085044`. */
...
/* Wave 54 (W54-D), sub_080860DC's callees. All four already have a promoted
 * body and the DEFINITIONS are what is published here: c_08037780.c,
 * c_0803BCD0.c, c_080876B4.c, c_08087B60.c. */
...
/* Same treatment, same reason: copied verbatim from the PROMOTED definition in
 * src/decomp/c_0800B5C0.c, which had no prototype. sub_0800B61C is the first
 * caller outside that translation unit (wave 56, W56-P). */
...
/* Wave 32 (W32-B): one `struct Unit *`, read off its own body -- it
 ...
 * struct Unit sub_08025BE0 initialises. */
 ...
 * it.
 *
 * Wave 56 (W56-F): the two installers themselves are now declared as well --
 * sub_08019F90 needs their addresses, and both are already DEFINED in promoted
 * src/decomp/c_08019D0C.c as `void f(ProcPtr)`, so these are transcribed from
 * the definitions rather than inferred from the call site. */
...
/* Wave 54 (W54-F). Both already have promoted bodies -- these declarations only
 * publish them, and both bodies were retyped in this wave from their single
 * caller sub_08076F34 (see the notes in src/decomp/c_08076F14.c and
 * src/decomp/c_08075298.c). sub_08076F14 is "is (x, y) inside a radius-16
 * circle": the caller truth-tests its result after `lsls #0x18`, so the return
 * is a byte, not the `int` the body used to carry. sub_08075298's a3/a4 are
 * SIGNED halfwords because that caller narrows both with `lsls #0x10; asrs
 * #0x10`; a5 is undiscriminated (its one call site passes 0). */
