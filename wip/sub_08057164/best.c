    ...
    for (i = 0; i <= 4; i++)
    {
        ...
        if (rows[gUnknown_03004582[y][0]].unk04 == 1)
        {
            if (y == gUnknown_0300450C)
                e->unk1a = gUnknown_0855203C[idx];
            else
                e->unk1a = gUnknown_0855203C[b * 10 + y * 5 + i];
        }
        else if (rows[gUnknown_03004582[y][0]].unk10 == 1)
            e->unk1a = gUnknown_08551F60[idx];
        else
            e->unk1a = gUnknown_08551E84[idx];

        e->unk00 = gUnknown_085521DC[y * 55 + a * 5 + i];
        e->unk01 = gUnknown_085521DC[y * 55 + b * 5 + i];
        e->unk02 = 0;
        e->unk04 = 0;
        e->unk06 = 0;
        idx++;
