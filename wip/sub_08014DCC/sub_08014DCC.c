/* WAVE 87 (W87-F) -- re-verified 71.7%, size-exact 120/120, first diff +0x6.
 * `do { } while (0)`: SIX placements, NEGATIVE -- three byte-identical, three
 * swapped best/bestSize AWAY from the ROM. It cannot create the live range the
 * residual needs. THE REAL FINDING: the ROM's `p = next` split and its
 * `q->used = next` tail reuse are ONE PACKAGE and they are MUTUALLY EXCLUSIVE
 * under agbcc -- the split survives only while `next` is dead at the loop exit,
 * and the tail reuse is available only while it is not. W77-G measured the tail
 * half alone and wave 42 measured the loop half alone; both closed an axis that
 * was never the axis. See work/sub_08014DCC/W87-notes.md for the five spellings.
 * NOTE: the wave-87 brief's "50.0%" for this park is wrong -- that is W77-G's
 * rejected probe, not the draft. */
#include "global.h"

/* PARKED, SIZE-EXACT at 120/120, 71.7% (wave 73, W73-E; re-measured wave 77, W77-G). Waves 40, 42 (W42-D),
 ...
 *
 * REMAINING RESIDUAL, now small and legible (34 of 120 bytes, first difference
 * at +0x6):
 ...
 *     valid probe nobody has run.
 *     WAVE 77 (W77-G) RAN IT AND IT IS NEGATIVE -- DO NOT RE-RUN IT. It goes
 *     120 -> 116 bytes (-4, 50.0%): gcc drops TWO instructions, not one. It
 *     const-propagates the zero through `next`, so the candidate loses the
 *     `movs r3,#0` WITHOUT gaining the ROM's `adds r2,r0,#0`. Read the hunk as
 *     a package rather than as a missed reuse: the ROM spends a copy of q into
 *     r2 and gets the `used` store for free out of r3; this candidate keeps q
 *     in r0 and pays a literal zero. Five instructions either way, and no
 *     spelling of the store alone moves which of the two you get.
 *
