 ...
 * RepaintTileRight @ 0x0800CEF8
 */

/* The tail half of MakeForestSimple with the first re-tile dropped: only the cell
 * to the RIGHT of (x, y) is re-tiled, and only when one exists.  Same guard,
 * one column narrower -- MakeForestSimple stops two columns from the edge
 * (`x < width - 2`) because it also touches x itself, this one stops one
 * column short.
 *
 * `MakeTileSimple(x, y, sub_080016D0(x, y))` is a genuine nesting and not two
