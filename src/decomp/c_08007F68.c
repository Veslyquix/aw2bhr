 ...
 * missing: the first `bl MakeTileSimple` sets r0 and r1 and leaves r2 alone, so
 ...
 * rewrite it as 1, then hand (x, y) to MakeForestSimple. IsTerrainAtCoordsType returns int
...
void sub_08007F68(int x, int y, int v)
{
    ...
    if (IsTerrainAtCoordsType(x, y2, 1))
        MakeTileSimple(x, y2, 1);
