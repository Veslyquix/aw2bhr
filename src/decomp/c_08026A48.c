 ...
 *
 * The two `ldr rN,=gPlayers` are one global read twice, not two
...
void sub_08026B28(void)
{
    ...
    for (i = 1; i <= 4; i++)
    {
        if (gPlayers[i].aiControlled != 0)
        {
            for (j = 1; j <= 4; j++)
            {
                if (i != j && gPlayers[i].team != gPlayers[j].team)
                    gPlayers[i].unk2c |= gUnknown_08090A84[j];
            }
        }
    }
}
