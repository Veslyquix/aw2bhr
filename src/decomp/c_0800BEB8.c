...
/* Two guards then an action, all on the same (x, y) cell: run SetTerrainAt
 * with terrain 7 only where IsTerrainAtCoordsType reports 0x13 and sub_0800BCD0 says
...
{
    if (IsTerrainAtCoordsType(a, b, 0x13) && !sub_0800BCD0(a, b))
        SetTerrainAt(a, b, 7);
}
