 ...
 * contiguous block at 0x0804D25C.
 ...
 * function is one call to sub_08011E54(src, dest, size) with size 0x200.
 *
 ...
 * MULT_EXPR, so `(u16)(x * 0x200)` becomes `lsl #25; lsr #0x10` in two
 * instructions while `(u16)(x << 9)` would need three (the shift, then the
