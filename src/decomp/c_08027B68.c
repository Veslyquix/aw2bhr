 ...
 * decrement here loads `ldrh` because force_to_mode drops the sign extension
 * when the only consumers are a halfword store and a `& 0xff`, while the
 * `== 0` test that follows keeps `ldrsh` because a SImode compare against 0
 * has no such mask. Declaring it u16 breaks the compare. */
struct Unk27B10Proc
{
    /* 00 */ PROC_HEADER;
